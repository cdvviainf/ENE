...
//
// Portado de FAS. A partir de un `HojaSpec[]` produce el libro de Carga
// Masiva: hoja de Instrucciones, hoja de ListasFijas (fuente de los
// dropdowns), y una hoja por maestro con encabezados, obligatorios
// resaltados, comentarios de ayuda y validaciones de lista (dropdowns) para
// enums y FKs internas.
...
function agregarHojaInstrucciones(wb: ExcelJS.Workbook, hojas: HojaSpec[], titulo: string, fueraDeAlcance: string[]) {
  ...
  const lineas: Array<[string, boolean]> = [
    ...
    ['3. Las columnas "Código" propias de Clientes, Proveedores y Servicios pueden dejarse vacías: el sistema genera el código automáticamente. El resto de los maestros usa código manual (no se autogenera).', false],
    ...
    ['8. Las columnas con varios códigos separados por coma (ej. Tipos de Servicio de un Proveedor) no tienen lista desplegable: escribe los códigos exactos, separados por coma.', false],
    ['9. Si vas a completar ejecutivos, alias, cuentas, contactos o direcciones para una fila, escribe su código a mano en vez de dejarlo vacío para autogenerar: el código autogenerado no se conoce hasta cargar, así que las hojas dependientes no podrían encontrarlo.', false],
  ]
  if (hayReferencia) {
    lineas.push(['10. Las hojas con pestaña gris son de SOLO REFERENCIA (datos ya existentes en el sistema): úsalas para copiar los códigos correctos en las hojas de datos. No las edites.', false])
  }
  lineas.push(['', false])
...
function aplicarValidacion(
  ...
  // fkMulti: Excel no soporta multiselección nativa; se deja texto libre y
  // la lista de códigos válidos va en el comentario del encabezado.
...
export interface OpcionesGenerar {
   ...
   * filas, cada fila mapeada por `campo`. Ej. { Comunas: [{ codigo, nombre }] }.
   */
...
export async function generarTemplate(hojas: HojaSpec[], opciones: OpcionesGenerar): Promise<ExcelJS.Buffer> {
  ...
  wb.creator = 'Extremo Norte Expediciones — Carga Masiva de Maestros'
  wb.created = new Date()
