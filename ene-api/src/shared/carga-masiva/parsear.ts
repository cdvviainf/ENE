...
//
// Portado de FAS. Convierte el libro en filas tipadas y coaccionadas,
// recolectando TODOS los errores estructurales (encabezado cambiado,
// requerido faltante, enum/boolean/número inválido). NO resuelve FKs a ids ni
// toca la BD: eso es responsabilidad del orquestador (que conoce el estado de
// la base y el orden de inserción).
//
// RN-DIN-01: las columnas `decimal` se devuelven como `string` (nunca
// `number`) — el orquestador del proyecto las pasa intactas al Zod schema
// correspondiente.
...
function coaccionar(
  ...
  switch (col.tipo) {
    ...
    case 'decimal': {
      ...
      // RN-DIN-01: se transporta como string, nunca number.
      return normalizado
    }
    case 'booleanSiNo': {
...
function parsearHoja(ws: ExcelJS.Worksheet | undefined, hoja: HojaSpec, erroresGlobales: ErrorFila[]): ResultadoHoja {
  ...
  // `actualRowCount` cuenta filas CON datos, no el último índice de fila —
  // una fila en blanco en medio de la hoja lo deja corto y trunca en
  // silencio las filas posteriores. `rowCount` (última fila tocada) es el
  // límite correcto para tolerar separadores en blanco dentro de los datos.
...
/**
 * Códigos declarados en la columna "codigo" de cada hoja, tal como aparecen en
 * el archivo (sin tocar la BD). La usan tanto `validarReferenciasInternas`
 * como el orquestador del proyecto: en dry-run, una FK `externo` que no
 * resuelve contra la BD puede igual resolver contra una fila de una hoja
 * anterior de este mismo archivo (ej. un Cliente nuevo + su Dirección en el
 * mismo Excel) — sin este mapa, el dry-run marcaría un falso FK_NO_RESUELTA.
 */
export function construirCodigosPorHoja(parseo: ResultadoParseo, hojas: HojaSpec[]): Record<string, Set<string>> {
  ...
  for (const hoja of hojas) {
    ...
    for (const fila of parseo.hojas[hoja.hoja]?.filas ?? []) {
      ...
      if (typeof cod === 'string') set.add(cod)
    }
    codigosPorHoja[hoja.hoja] = set
  }
  return codigosPorHoja
}
 ...
 */
export function validarReferenciasInternas(
  parseo: ResultadoParseo,
  hojas: HojaSpec[],
  codigosPorHoja: Record<string, Set<string>> = construirCodigosPorHoja(parseo, hojas),
): ErrorFila[] {
  const errores: ErrorFila[] = []
  for (const hoja of hojas) {
  ...
  for (const hoja of hojas) {
    // Solo FKs internas simples (resuelven contra el propio archivo). Las
    // marcadas `externo` referencian maestros que ya existen en BD (aunque
    // tengan `hoja` para el dropdown) y se resuelven en el commit, no acá.
    // `fkMulti` no se valida acá: sus múltiples códigos se resuelven uno a
    // uno en el commit (mismo tratamiento que una FK externa).
