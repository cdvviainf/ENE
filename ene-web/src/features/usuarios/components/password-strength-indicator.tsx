...
export function PasswordStrengthIndicator({ password }: { password: string }) {
  const checks = useMemo(() => PASSWORD_RULES.map((rule) => ({ label: rule.label, passes: rule.passes(password) })), [password]);
  ...
  return (
    <ul className='mt-2 space-y-1'>
      {checks.map((check) => (
        <li
          key={check.label}
          className={cn('flex items-center gap-1.5 text-xs', check.passes ? 'text-green-600' : 'text-muted-foreground')}
        >
          {check.passes ? (
            <Icons.check className='h-3.5 w-3.5 shrink-0 text-green-600' />
          ) : (
            <Icons.close className='text-muted-foreground h-3.5 w-3.5 shrink-0' />
          )}
          {check.label}
        </li>
      ))}
