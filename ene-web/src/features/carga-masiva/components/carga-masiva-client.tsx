...
import { cargaMasivaService } from '../service';
...
export function CargaMasivaClient() {
  ...
  const templateMut = useMutation({
    mutationFn: () => cargaMasivaService.descargarTemplate(),
    onError: (e: Error) => toast.error(e.message || 'No se pudo descargar la plantilla')
  });

  const procesarMut = useMutation({
    mutationFn: (commit: boolean) => cargaMasivaService.procesar(archivo!, commit),
  ...
  const reporteMut = useMutation({
    mutationFn: () => cargaMasivaService.descargarReporte(archivo!),
  ...
  if (!puedeLeer) {
    return (
      <div className='text-muted-foreground text-center text-lg'>No tienes acceso a esta página.</div>
    );
  }
    ...
    <div className='flex flex-col gap-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Descarga el Excel base vacío, complétalo y vuelve a subirlo. Los códigos de Cliente, Proveedor y Servicio
            se pueden dejar vacíos: se autogeneran. El resto de los maestros usa código manual.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {resultado && (
        <Card>
          <CardHeader>
            ...
            {resultado.dryRun && <CardDescription>Validación — no se escribió nada en la base.</CardDescription>}
          </CardHeader>
          <CardContent className='flex flex-col gap-6'>
            ...
            {resultado.errores.length > 0 ? (
              ...
              <Alert>
                ...
                <AlertDescription>
                  {resultado.dryRun ? 'El archivo está listo para cargar.' : 'Todos los registros se cargaron correctamente.'}
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
