...
// cargaMasivaRoutes sin prefijo propio) → segmento "config/" explícito.
...
export const cargaMasivaService = {
  ...
  async descargarTemplate(): Promise<void> {
    const blob = await api.get('config/carga-masiva/template').blob();
    descargarBlob(blob, 'Carga_Masiva_Maestros_ENE_base.xlsx');
  },
  ...
  async descargarReporte(archivo: File): Promise<void> {
    ...
    const blob = await api
      .post('config/carga-masiva/reporte', { body: form, timeout: 120_000 })
      .blob();
