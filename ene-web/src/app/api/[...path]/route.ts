...
// nunca el de ene-api directamente. Sin esto, la cookie de sesión (seteada
// para el dominio del frontend por /api/auth/[...all]/route.ts) no llega a
// un dominio distinto -- crítico cuando ene-web y ene-api viven en
...
const API_BASE =
  ...
  (process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3011/api').replace(/\/api$/, '')
